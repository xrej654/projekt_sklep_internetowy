...
<body>
    <section class="tabelka-panelu">
        ...
        echo "<table>";
        echo "<tr> <th>Producent</th> <th>Czy dodac promocje</th> <tr>";

        //wyswieltnaie producentow
        ...
        //dodawanie promocji wzgledem producenta
        if (isset($_POST['submit'])) {
            $query = "SELECT * FROM `produkt` WHERE producent_id IN(" . implode(',', $_POST['ids']) . ")";
            $result = $connection->query($query);

            while ($row = $result->fetch_assoc()) {
                $query = "SELECT * FROM `promocja_produkt` WHERE promocja_id = {$_POST['promocja']} AND produkt_id = {$row['produkt_id']}";
                $relations = $connection->query($query);

                if ($relations->num_rows == 0) {
                    $query = "INSERT INTO `promocja_produkt` (promocja_id, produkt_id) VALUES ({$_POST['promocja']}, {$row['produkt_id']})";
